...
> run (courses 1-3 + Scan-the-Code loiter) with a leak-proof raw-level split,
> all three classes including blue, held-out mAP50 0.991, and honestly stated
> caveats. The balloon-photo results below (`results/`) remain the only
> real-imagery baseline. The pipeline now splits BEFORE augmentation via
> `scripts/00_preprocess_training_data.py`; the flow described further down
> this README predates that fix (see `docs/08_annotation_and_training.md` for
